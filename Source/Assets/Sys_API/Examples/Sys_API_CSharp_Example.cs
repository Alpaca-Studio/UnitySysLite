...
public class Sys_API_CSharp_Example : MonoBehaviour {
	...
	public void Awake () {
		...
		dir = Sys.IO.DeviceExternalStorage();
		dataPath = dir + "/" + Application.productName + "/Logs2";
		if(Sys.IO.DirectoryCheck(dataPath,true)){
			Sys.IO.FileCheck(dataPath+"/", logFileName, true);
		}
	}
	void Start () {
		Sys.Logging.Log("This is a test..");
		Sys.Logging.Log("This is a test...",true);
		Sys.Logging.Log("This is a test....");
		Sys.Logging.Log("This is a test.....", true);
		Sys.Logging.Log("This is a test......");
		Sys.Logging.Log("This is a test.......",true);
		...
		Sys.Logging.Log("This is a test..............");
		Sys.Logging.Log("This is a test...............", true);
		Sys.Logging.Log("This is a test................", false);
		logRaw = Sys.Logging.logRaw;
		logData = Sys.Logging.logData;
		//Sys.SaveLog(dataPath + "/" + logFileName, true);
		//Sys.SaveSystemInfo(dataPath + "/" + sysFileName);
		//uniqueID = Sys.GenerateUniqueID(21);
		cProduct = Sys.Math.cProduct(a,b);
	...
	void Update () {
		...
		if(CaptureScreenView){
			...
			Sys.ScreenShot.CaptureScreenshot(this, (dataPath + "/" + getDateTime + ".png"), true);
		...
		if(SaveArrayToFile){
			...
			Sys.IO.SaveFile(dataPath + "/" + "abc.txt", saveArray);
			SaveArrayToFile = false;
		}
		if(GetArrayFromFile){
			//loadArray = Sys.LoadDataFromFile(dataPath + "/" + "abc.txt");
			loadArray = Sys.IO.ReadFile(dataPath + "/" + "abc.txt");
			GetArrayFromFile = false;
		}
		
		if(SaveSystemInfo){
			Sys.Info.SaveSystemInfo(dataPath + "/" + sysFileName, true);
			SaveSystemInfo = false;
		}
		
		if(GetSystemInfo){
			SystemInfo = new List<string>(Sys.Info.GetSystemInfo());
			GetSystemInfo = false;
		}
		
		if(SaveSystemLog){
			Sys.Logging.SaveLog(dataPath + "/" + logFileName, true);
			SaveSystemLog = false;
		}
		
		if(ClearSystemLog){
			Sys.Logging.ClearLog();
			ClearSystemLog = false;
		}
		
		if(GenerateUniqueID){
			uniqueID = Sys.M.GenerateUniqueID(uIDLength);
